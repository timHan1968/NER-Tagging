...
class BigramLM(UnigramLM):
	...
	def calBiProb(self, firstWord, secondWord):
		"""
		a function that returns probability of a given bigram without
		smoothing.
		"""
		if secondWord not in self._separatedbigrams[firstWord]:
			num = 0
		else:
			num = self._separatedbigrams[firstWord][secondWord]

		den = self._tokens[firstWord]
		return num/den
...
#def main():
	...
	#printUniTable(xx_o, "UnigramObamaTable.txt")
	#printUniTable(xx_t, "UnigramTrumpTable.txt")
	#printBiTable(tt_o, "BigramObamaTable.txt")
	#printBiTable(tt_t, "BigramTrumpTable.txt")

	#unseenSpeech = readTest("Assignment1_resources/test/test.txt")
	#speechClassify(tt_o, tt_t, unseenSpeech)
